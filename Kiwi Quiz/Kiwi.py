...
print "Vennligst ta i bruk spillere.txt vedlegg for a legge til lagspill. \n"
print "-- Skriv quit nor du vil avslutte a lagre dine poeng. --\n"
...
def save_score(score):
    with open("score\score.txt", mode='a') as file:
        file.write('%sms: %s\n' %       
            (datetime.datetime.now(), ""))
        file.write(str("Du fikk poeng: "))
        file.write(str(score)), file.write(str("\n\n"))
        file.close()

while loop1 == True:
    open("spillere.txt", "w").write(str())
    try:
    ...
    for i in range(spillere):
        ...
        if navn in ["quit", "Quit", "QUIT"]:
            loop1 = False
            gameover = True
            break
        deltakere.append(navn)
        f = open(filepath("spillere.txt"), "w")
        f.write("Spillere: \n"), f.write("\n")
        lst = '\n'.join(deltakere)
        f.write(lst)
        f.write("\n\n"), f.write("Lag: ")
...
while loop2 == True:
    ...
    PLU = ["Banan", "Agurk", "Rode Epler", "Klementiner", "Gule Epler", "Gronne Epler",
           "Varlok", "Smagodt i pose", "Smagodt i beger", "Blomkal", "Brokkoli", "Tomater Losvekt",
           "Grillkull", "Sellerirot", "Pastinakk", "Persillerot", "Notter Losvekt", "Mandler Losvekt",
           "Pepsi Max 4pk", "Coca Cola 4pk", "Ananas", "Kokosnott", "Parer", "Asparges", "Aubergine", "Rodbeter Bunt 3pk",
           ...
           "Isberg Salat", "Rode Druer Losvekt", "Biff Tomat", "Epler Norske", "Plommer Norske"]
    chosen_word = random.choice(PLU)
    if score > 0:
        print "Score:", score
    print "Oppgave:", oppgave
    print chosen_word
    svar = ["4011", "4316", "4015", "4055", "4021", "4139", "4068", "9009", "509", "4079", "4349", "4959",
            ...
            "3101", "4317", "4326", "4745", "4033", "4197", "4276", "4810", "4041", "4740", "4554", "4539", "3061",
    ...
    if guess in svar:
        if guess == "4011":
            ...
            if chosen_word == "Tomater Losvekt":
                print"Svar = Tomater Losvekt, Riktig"
                print ""
                score += 1
            else:
                feil()
        ...
        if guess in "4049":
            ...
            if chosen_word == "Rodbeter Bunt 3pk":
                print"Svar = Rodbeter Bunt 3pk, Riktig"
                print ""
                score += 1
            else:
                feil()
        elif guess == "3061":
            if chosen_word == "Biff Tomat":
                print"Svar = Biff Tomat, Riktig"
                print ""
                score += 1
            else:
                feil()
        elif guess == "4204":
            if chosen_word == "Epler Norske":
                print"Svar = Epler Norske, Riktig"
                print ""
                score += 1
            else:
                feil()
        elif guess == "4405":
            if chosen_word == "Plommer Norske":
                print"Svar = Plommer Norske, Riktig"
                print ""
                score += 1
            else:
                feil()
